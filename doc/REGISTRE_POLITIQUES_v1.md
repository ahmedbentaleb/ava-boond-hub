...
| `ref_critere_evaluation` | — | relationnelle, technique | ⭐ **grille de parité, 24/09** — oubliée le 23/09 |
| `ref_pole` | — | pole_rh, pole_sales | ⭐ 24/09 — sur candidat, ressource, besoin, compte |
| `ref_mobilite` | — | ⬜ à remplir par l'admin (zones) | ⭐ 24/09 |
| `ref_etat_paiement` | `planifie` · `confirme` · `regle` | planifie, confirme, regle | ⭐ 24/09 soir — **V-121** : l'état figé en CHECK dans 012 |
...
| `ui.liste.positionnements.colonnes` | **les 7 de Boond, dans l'ordre** · toute sélection parmi les 9 | grille de parité, 24/09 | ⭐ la liste manquait : Boond la configure comme les autres |

⭐ **Grille de parité, 24/09** : chaque `ui.liste.*.colonnes` propose **toutes** les colonnes relevées chez Boond (`cartographie/BOOND_PARITE_2026-09-24.md` §1), affichées **et** disponibles ; le défaut devient celui de Boond. Réglable compte par compte.
...
| ⭐ **Énumérations techniques en CHECK, justifiées (V-132, 24/09)** : `envoi_email.nature` (message · push_cv), `lien_outlook.nature` (evenement · message), `document_genere.format` (docx · pdf), `perimetre.type_code` (D-9) | le **code** s'y branche ; un administrateur n'a rien à y ajouter. ⛔ Un **état** ou un **type métier** n'est jamais ici : il a son `ref_*` |
| ⭐ **Et quatre de plus, trouvées en mesurant la base (V-132, 24/09 nuit)** : `politique.type` (enum · int · bool · list · decimal · couleur · texte) et `politique.categorie` (metier · installation · apparence) — la forme d'un réglage, que l'écran d'administration lit pour savoir comment l'afficher ; `tentative_refusee.code` (DROIT · ETAT · GARDE · MUR · INTROUVABLE) — les cinq familles de refus du **contrat L4**, pas une liste métier ; `calendrier_jour_non_ouvre.motif` (week_end · ferie · fermeture) — le générateur de calendrier s'y branche | même règle : le code s'y branche, un administrateur n'a rien à y ajouter. ⭐ La liste complète se **mesure en base** (grille B5), jamais dans les fichiers : une migration qui retire un CHECK laisse son texte dans le fichier d'origine |
...
awk -F'|' '/^## C\./{c=1} /^## D\./{c=0} c && /^\| `/ {print $2}' _ops/REGISTRE_POLITIQUES_v1.md \
  | tr -d '\r' | grep -oE '`[a-z0-9_.]+`' | sort -u | wc -l     # 201 en tableau au 25/09 (§C + C.bis + C.ter), + 1 en prose (ui.theme.personnalise) = 202
# ⭐ Et contre la base — la seule mesure qui départage :
psql -h 127.0.0.1 -U postgres -d ava -Atc "select count(*) from ava.politique"   # = le compte ci-dessus ; ⛔ V-143 : ce commentaire disait encore 173 le 25/09
```
